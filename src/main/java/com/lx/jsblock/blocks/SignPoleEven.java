...
public class SignPoleEven extends HorizontalFacingBlock {
    public static final IntProperty LENGTH = IntProperty.of("length", 2, 7);
    public SignPoleEven(Settings settings) {
        super(settings);
        setDefaultState(getStateManager().getDefaultState().with(LENGTH, 2));
    }
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        /* Follow the rotation of the block below if its even signs, or this pole */

        if(blockBelow.isOf(Blocks.RAILWAY_SIGN_2_EVEN)) {
            return getDefaultState().with(FACING, IBlock.getStatePropertySafe(blockBelow, FACING)).with(LENGTH, 2);
        }
        if(blockBelow.isOf(Blocks.RAILWAY_SIGN_3_EVEN)) {
